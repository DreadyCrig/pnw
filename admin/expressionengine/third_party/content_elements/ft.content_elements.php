...
class Content_Elements_ft extends EE_Fieldtype {
	...
	public function display_field($data) {
		...
		if (!is_array($data)) {			
			...
			// Publisher issue - data in publisher has been not corrupted and not need fix it			
			if ($data === FALSE && $raw)
			{
...
/**
 * Total element count (by element type)
 * Autor: Milan Topalov, 22/10/2014
 *
 ...
 * @return string
 */	
	public function replace_total_elements($data, $params = array(), $tagdata = FALSE)
	{			
		$data = $this->_compatibilty_mode($data);
		...
		{
			$count = 0;
			$interest_elements = explode("|", $params['element_type']);		

			foreach ($data as $key => $element)
			{
				if (in_array($element['element_type'], $interest_elements))
				{
					$count++;
				}			
			}	
