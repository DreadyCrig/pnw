...
class Low_events_ft extends EE_Fieldtype {
	...
	private function _display_settings($settings = array())
	{
		...
		$it[] = array(
			lang('le_first_day'),
			form_dropdown('first_day', array_map('lang', $this->_dotw), $settings['first_day'])
		);
		...
		// Hide all day?
		$it[] = array(
			lang('le_hide_all_day'),
			low_yes_no('hide_all_day', $settings['hide_all_day'])
		);
	...
	public function display_field($data)
	{
		...
		$data['hide_all_day'] = (@$this->settings['hide_all_day'] == 'y');
		...
		$data['data'] = array(
			'first-day'     => (int) @$this->settings['first_day'],
			'time-format'   => $fmt,
	...
	*/
	...
	private function _load_assets()
	{
		...
		{
			// location on server
			$file_url = $asset_url.$file.'?v='.(LOW_EVENTS_DEBUG ? time() : LOW_EVENTS_VERSION);
