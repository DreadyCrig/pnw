<?php
	...
	<?php else :
		...
		foreach($text_settings as $setting)
		{
			...
			elseif(!empty($current['settings'][$setting]))
			{
			...
			else
			{
				$v = ($setting == 'cache_path') ?
					PATH_THIRD_THEMES.'cp_analytics/' : 
					URL_THIRD_THEMES.'cp_analytics/';
			}
			$this->EE->table->add_row(
		...
		foreach($hidden_settings as $setting)
		{
			echo form_hidden($setting, (!empty($current['settings'][$setting])) ? $current['settings'][$setting] : '');
		}
