...
class Cp_analytics_upd {
	...
	public function install()
	{
		...
		$data = array(
			...
			'method' => 'integration_statusboard'
		);
		$this->EE->db->insert('exp_actions', $data);
	...
	public function update($current = '')
	{
		if(version_compare($current, '2.1', '<'))
		{
			...
			// Truncate (new storage format, better tokens)
			$this->EE->db->truncate('exp_cp_analytics');
		}
		
		if(version_compare($current, '2.2', '<'))
		{
			// Add action for grabbing realtime data
			$data = array(
				'class' => $this->module_name,
				'method' => 'get_realtime_data'
			);
